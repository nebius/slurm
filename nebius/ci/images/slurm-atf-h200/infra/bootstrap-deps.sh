...
# Image-build-only provisioning. Runtime ATF must use verify-image.sh and must
# never install or update host dependencies on a disposable validation VM.
...
# Valgrind is used only by the explicit `test39.18 valgrind` diagnostic mode;
# the full ATF suite does not enable it. Some Nebius CUDA images cannot resolve
# valgrind because their Ubuntu repositories do not provide the matching
# libc6-dbg build. Keep it when installable, but never block the immutable test
# image on this optional debugger.
if apt-get --simulate install valgrind >/dev/null 2>&1; then
	sudo DEBIAN_FRONTEND=noninteractive apt-get install -y valgrind
else
	echo "Optional valgrind skipped: a compatible libc6-dbg is unavailable."
fi
...
if [[ "${xnvctrl_candidate}" == "(none)" ]] &&
	...
	gpu_driver_version="$(
		nvidia-smi --query-gpu=driver_version --format=csv,noheader |
			awk 'NR == 1 {version = $0} END {print version}'
	)"
	xnvctrl_version="$(
		apt-cache madison libxnvctrl0 |
			awk -v prefix="${gpu_driver_version}-" \
				'index($3, prefix) == 1 && version == "" {version = $3}
				 END {print version}'
	)"
