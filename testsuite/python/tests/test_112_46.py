...
def test_loaded_versions():
    ...
    assert "/slurm/v0.0.46/jobs/" in spec["paths"].keys()
    assert "/slurmdb/v0.0.46/jobs/" in spec["paths"].keys()
...
def test_db_accounts(slurm, slurmdb, create_wckeys, admin_level):
    ...
    from openapi_client.models.v0046_account import V0046Account
    ...
    from openapi_client.models.v0046_openapi_accounts_resp import (
        V0046OpenapiAccountsResp,
    )

    # make sure account doesn't already exist
    resp = slurmdb.slurmdb_v0046_get_account_with_http_info(account_name)
    assert resp.status_code == 200
    assert len(resp.data.accounts) == 0
    resp = slurmdb.slurmdb_v0046_get_account_with_http_info(account2_name)
    assert resp.status_code == 200
    assert len(resp.data.accounts) == 0

    # create account
    accounts = V0046OpenapiAccountsResp(
        accounts=[
            V0046Account(
    ...
    resp = slurmdb.slurmdb_v0046_post_accounts_with_http_info(accounts)
    assert resp.status_code == 200
    ...
    resp = slurmdb.slurmdb_v0046_get_account(account2_name)
    ...
    resp = slurmdb.slurmdb_v0046_post_accounts(accounts)
    ...
    resp = slurmdb.slurmdb_v0046_get_account(account_name)
    assert resp.accounts
    for account in resp.accounts:
        assert account.name == account_name

    # check full listing works
    resp = slurmdb.slurmdb_v0046_get_accounts(deleted="true")
    assert resp.accounts
    resp = slurmdb.slurmdb_v0046_get_accounts()
    assert resp.accounts
    ...
    resp = slurmdb.slurmdb_v0046_delete_account(account_name)
    assert not resp.warnings
    assert len(resp.errors) == 0

    resp = slurmdb.slurmdb_v0046_delete_account(account2_name)
    assert not resp.warnings
    assert len(resp.errors) == 0
...
def test_db_wckeys(slurmdb, create_coords, admin_level):
    from openapi_client.models.v0046_openapi_wckey_resp import V0046OpenapiWckeyResp
    from openapi_client.models.v0046_wckey import V0046Wckey

    wckeys = V0046OpenapiWckeyResp(
        wckeys=[
            V0046Wckey(
    ...
    resp = slurmdb.slurmdb_v0046_post_wckeys_with_http_info(
        v0046_openapi_wckey_resp=wckeys
    )
    assert resp.status_code == 200

    resp = slurmdb.slurmdb_v0046_get_wckeys()
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert len(resp.wckeys) >= 1

    resp = slurmdb.slurmdb_v0046_get_wckey(wckey_name)
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.wckeys
    ...
    resp = slurmdb.slurmdb_v0046_get_wckey(wckey2_name)
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.wckeys
    for wckey in resp.wckeys:
    ...
    resp = slurmdb.slurmdb_v0046_delete_wckey(wckey_name)
    assert not resp.warnings
    assert len(resp.errors) == 0

    resp = slurmdb.slurmdb_v0046_delete_wckey(wckey2_name)
    assert not resp.warnings
    assert len(resp.errors) == 0
...
def test_db_clusters(slurmdb, admin_level):
    ...
    clusters = V0046OpenapiClustersResp(
        clusters=[
            V0046ClusterRec(
                name=cluster_name,
            ),
    ...
    resp = slurmdb.slurmdb_v0046_get_clusters()
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.clusters

    resp = slurmdb.slurmdb_v0046_get_cluster(cluster_name)
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.clusters
    ...
    resp = slurmdb.slurmdb_v0046_get_cluster(cluster2_name)
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.clusters
    for cluster in resp.clusters:
    ...
    resp = slurmdb.slurmdb_v0046_delete_cluster(cluster_name)
    assert len(resp.errors) == 0

    resp = slurmdb.slurmdb_v0046_delete_cluster(cluster2_name)
    assert len(resp.errors) == 0
...
def test_db_users(slurmdb, admin_level):
    from openapi_client.models.v0046_assoc_short import V0046AssocShort
    from openapi_client.models.v0046_coord import V0046Coord
    from openapi_client.models.v0046_openapi_users_resp import V0046OpenapiUsersResp
    from openapi_client.models.v0046_user import V0046User
    from openapi_client.models.v0046_user_default import V0046UserDefault
    from openapi_client.models.v0046_wckey import V0046Wckey

    users = V0046OpenapiUsersResp(
        users=[
            V0046User(
    ...
    resp = slurmdb.slurmdb_v0046_post_users(v0046_openapi_users_resp=users)
    assert not resp.warnings
    assert len(resp.errors) == 0

    resp = slurmdb.slurmdb_v0046_get_users()
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.users
    ...
    resp = slurmdb.slurmdb_v0046_get_user(user_name, with_wckeys="true")
    if resp.warnings:
    ...
    resp = slurmdb.slurmdb_v0046_get_user(coord_name, with_wckeys="true")
    if resp.warnings:
        assert len(resp.warnings) == 1
        assert resp.warnings[0].source == "#/with_wckeys/"
    assert len(resp.errors) == 0
    ...
    resp = slurmdb.slurmdb_v0046_delete_user(coord_name)
    assert len(resp.errors) == 0

    user_exists = False
    resp = slurmdb.slurmdb_v0046_get_user(coord_name, with_deleted="true")
    assert len(resp.errors) == 0
    for user in resp.users:
        assert user.name == coord_name
    ...
    if not user_exists:
        ...
        resp = slurmdb.slurmdb_v0046_get_user(coord_name)
        assert len(resp.warnings) > 0
        assert len(resp.errors) == 0
...
def test_db_assoc(slurmdb, create_coords, create_qos, admin_level):
    ...
    associations = V0046OpenapiAssocsResp(
        associations=[
            V0046Assoc(
                ...
                min=dict(
                    priority_threshold=V0046Uint32NoVal(
                        set=True,
                        number=10,
                    )
                ),
    ...
    resp = slurmdb.slurmdb_v0046_get_associations()
    assert not resp.warnings
    assert len(resp.errors) == 0
    ...
    associations = V0046OpenapiAssocsResp(
        associations=[
            V0046Assoc(
                ...
                max=dict(
                    jobs=dict(
                        per=dict(wall_clock=V0046Uint32NoVal(set=True, number=250)),
                    ),
                ),
                min=dict(
    ...
    resp = slurmdb.slurmdb_v0046_delete_associations(
        cluster=local_cluster_name,
        user=user_name,
    )
    assert len(resp.errors) == 0
...
def test_db_qos(slurmdb, create_coords, admin_level):
    from openapi_client.models.v0046_float64_no_val_struct import (
        V0046Float64NoValStruct as V0046Float64NoVal,
    )
    ...
    qos = V0046OpenapiSlurmdbdQosResp(
        qos=[
            V0046Qos(
                ...
                name=qos_name,
                preempt=dict(
                    exempt_time=V0046Uint32NoVal(set=True, number=199),
                ),
                priority=V0046Uint32NoVal(number=180, set=True),
                usage_factor=V0046Float64NoVal(
                    set=True,
                    number=82382.23823,
                ),
                usage_threshold=V0046Float64NoVal(
                    set=True,
                    number=929392.33,
                ),
            ),
    ...
    resp = slurmdb.slurmdb_v0046_get_qos()
    assert not resp.warnings
    assert len(resp.errors) == 0
    assert resp.qos
    ...
    resp = slurmdb.slurmdb_v0046_get_single_qos(qos2_name)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    assert resp.qos
    for qos in resp.qos:
    ...
    resp = slurmdb.slurmdb_v0046_delete_single_qos(qos_name)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    ...
    resp = slurmdb.slurmdb_v0046_delete_single_qos(qos2_name)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
...
def test_db_tres(slurmdb):
    resp = slurmdb.slurmdb_v0046_get_tres()
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0


def test_db_config(slurmdb, admin_level):
    resp = slurmdb.slurmdb_v0046_get_config()
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
...
def test_jobs(slurm, slurmdb, non_admin):
    ...
    from openapi_client.models.v0046_job_info import V0046JobInfo
    from openapi_client.models.v0046_job_modify import V0046JobModify
    from openapi_client.models.v0046_job_modify_tres import V0046JobModifyTres
    from openapi_client.models.v0046_job_submit_req import V0046JobSubmitReq
    from openapi_client.models.v0046_openapi_job_modify_req import (
        V0046OpenapiJobModifyReq,
    )
    from openapi_client.models.v0046_process_exit_code_verbose import (
        V0046ProcessExitCodeVerbose,
    )
    from openapi_client.models.v0046_process_exit_code_verbose_signal import (
        V0046ProcessExitCodeVerboseSignal,
    )
    from openapi_client.models.v0046_tres import V0046Tres
    from openapi_client.models.v0046_uint32_no_val_struct import V0046Uint32NoValStruct
    from openapi_client.models.v0046_uint32_no_val_struct import (
        V0046Uint32NoValStruct as V0046Uint32NoVal,
    )
    ...
    resp = slurm.slurm_v0046_post_job_submit(job)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    assert resp.job_id
    assert resp.step_id
    ...
    resp = slurm.slurm_v0046_get_jobs()
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    ...
    resp = slurm.slurm_v0046_get_job(str(jobid1))
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    for job in resp.jobs:
        assert job.job_id == jobid1
    ...
    resp = slurm.slurm_v0046_get_job(str(jobid2))
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    ...
    job = V0046JobSubmitReq(
        ...
        job=V0046JobDescMsg(
            ...
            name="test job",
            environment=env,
            priority=V0046Uint32NoVal(number=0, set=True),
            current_working_directory="/tmp/",
        ),
    )
    ...
    resp = slurm.slurm_v0046_post_job(str(jobid3), v0046_job_desc_msg=job)
    ...
    resp = slurm.slurm_v0046_get_job(str(jobid3))
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    for job in resp.jobs:
    ...
    resp = slurm.slurm_v0046_delete_job(str(jobid3))
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    ...
    WIFEXIT_CODE = V0046Uint32NoValStruct(
        set=True, infinite=False, number=4  # 1024 >> 8
    )

    job_modify = V0046JobModify(
        comment=V0046JobComment(
            administrator="admin1",
            job="job1",
            system="system1",
        ),
        derived_exit_code=V0046ProcessExitCodeVerbose(
            return_code=V0046Uint32NoValStruct(set=True, infinite=False, number=1024),
        ),
        extra="extra1",
        tres=V0046JobModifyTres(
            allocated=[V0046Tres(type="energy", count=12346, id=3)]
        ),
        wckey="mywckey1",
    )

    resp = slurmdb.slurmdb_v0046_post_job(str(jobid1), v0046_job_modify=job_modify)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0

    resp = slurmdb.slurmdb_v0046_get_jobs(step=str(jobid1))
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    assert len(resp.jobs) == 1
    for job in resp.jobs:
    ...
    job_modify = V0046JobModify(
        ...
        derived_exit_code=V0046ProcessExitCodeVerbose(
            return_code=V0046Uint32NoValStruct(set=True, infinite=False, number=2048),
        ),
        extra="extra2",
        tres=V0046JobModifyTres(
            allocated=[V0046Tres(type="energy", count=54321, id=3)]
        ),
        wckey="mywckey2",
    )

    job_modify_req = V0046OpenapiJobModifyReq(
        job_id_list=[str(jobid1), str(jobid2)], job_rec=job_modify
    )
    resp = slurmdb.slurmdb_v0046_post_jobs(v0046_openapi_job_modify_req=job_modify_req)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0

    resp = slurmdb.slurmdb_v0046_get_jobs(job_name="runjob")
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
...
def test_partitions(slurm):
    resp = slurm.slurm_v0046_get_partition(partition_name)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    ...
    resp = slurm.slurm_v0046_get_partitions()
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    assert resp.partitions
...
def test_post_partitions(slurm, admin_level):
    ...
    from openapi_client.models.v0046_partition_info import V0046PartitionInfo
    from openapi_client.models.v0046_partition_info_defaults import (
        V0046PartitionInfoDefaults,
    )
    from openapi_client.models.v0046_partition_info_nodes import V0046PartitionInfoNodes
    ...
    req = V0046OpenapiPartitionsModReq(
        partitions=[
            V0046PartitionInfo(
                name=part_1_name,
                nodes=V0046PartitionInfoNodes(configured=part_1_nodes),
                flags=[
                    "HIDDEN",
                    "NO_ROOT",
                    "ROOT_ONLY",
                ...
                defaults=V0046PartitionInfoDefaults(
                    partition_memory_per_cpu=V0046Uint64NoValStruct(
                        set=True,
                        number=10,
                    ),
                ),
            ...
            V0046PartitionInfo(
                name=part_2_name,
                nodes=V0046PartitionInfoNodes(configured=part_2_nodes),
                flags=[
                    "LLN",
                ],
                preempt_mode=["DISABLED"],
                defaults=V0046PartitionInfoDefaults(
                    partition_memory_per_node=V0046Uint64NoValStruct(
                        set=True,
                        number=12,
                    )
                ),
...
def test_delete_partition(slurm, admin_level):
    ...
    resp = slurm.slurm_v0046_delete_partition(partition_name)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
...
def test_nodes(slurm, admin_level):
    from openapi_client.models.v0046_update_node_msg import V0046UpdateNodeMsg
    ...
    resp = slurm.slurm_v0046_post_node(node_name, v0046_update_node_msg=node)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0

    resp = slurm.slurm_v0046_get_node(node_name)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
    assert resp.nodes
    for node in resp.nodes:
    ...
    resp = slurm.slurm_v0046_post_node(node_name, node)
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
...
def test_diag(slurm):
    resp = slurm.slurm_v0046_get_diag()
    assert len(resp.warnings) == 0
    assert len(resp.errors) == 0
...
def test_licenses(slurm):
    resp = slurm.slurm_v0046_get_licenses()
    assert len(resp.errors) == 0
...
def test_reservations(slurm, flags, admin_level):
    from openapi_client.models.v0046_reservation_desc_msg import V0046ReservationDescMsg
    from openapi_client.models.v0046_reservation_mod_req import V0046ReservationModReq
    from openapi_client.models.v0046_uint32_no_val_struct import V0046Uint32NoValStruct
    from openapi_client.models.v0046_uint64_no_val_struct import V0046Uint64NoValStruct

    resv_name = "test_resv"
    users = ["root", "atf"]
    duration = V0046Uint32NoValStruct(number=300, set=True)
    start_time = V0046Uint64NoValStruct(number=int(time.time()) + 60, set=True)
    end_time = V0046Uint64NoValStruct(
    ...
    reservation_info = V0046ReservationDescMsg(
        name=resv_name,
        users=users,
        duration=duration,
    ...
    resp = slurm.slurm_v0046_post_reservations(
        V0046ReservationModReq(reservations=[reservation_info])
    )
    ...
    resp = slurm.slurm_v0046_get_reservation(resv_name)
    ...
    new_users = ["root"]
    new_node_list = ["node2"]
    new_start_time = V0046Uint64NoValStruct(number=int(time.time()) + 90, set=True)
    new_flags = ["IGNORE_JOBS", "MAGNETIC", "USER_DELETE"]

    update_info = V0046ReservationDescMsg(
        name=resv_name,
        end_time=V0046Uint64NoValStruct(set=True, number=new_end_time),
        users=new_users,
        node_list=new_node_list,
        start_time=new_start_time,
        flags=new_flags,
    )
    resp = slurm.slurm_v0046_post_reservation(update_info)
    ...
    # Delete reservation
    resp = slurm.slurm_v0046_delete_reservation(reservation_name=resv_name)
    assert not resp.warnings and not resp.errors
    assert resv_name not in [
        r.name for r in slurm.slurm_v0046_get_reservations().reservations
...
def test_legal_node_creation(slurm, admin_level, legal_state, dynamic_node):
    from openapi_client.models.v0046_openapi_create_node_req import (
        V0046OpenapiCreateNodeReq,
    )

    request_body = V0046OpenapiCreateNodeReq(
        node_conf=f"nodename={dynamic_node} State={legal_state}"
    )

    response = slurm.slurm_v0046_post_new_node(
        v0046_openapi_create_node_req=request_body
    )
...
def test_illegal_node_creation(slurm, admin_level, illegal_state, dynamic_node):
    ...
    try:
        ...
        slurm.slurm_v0046_post_new_node_with_http_info(
            v0046_openapi_create_node_req=request_body
        )
        assert (
            False
        ), "slurm_v0046_post_new_node_with_http_info should have raised an exception"
    except ApiException as e:
        response = slurm.api_client.deserialize(
            response=ApiResponse(data=e.body), response_type="V0046OpenapiResp"
        )
...
def test_util_hostnames(util_api):
    from openapi_client.models.v0046_openapi_hostlist_req_resp import (
        V0046OpenapiHostlistReqResp,
    )

    hostnames = ["node01", "node02", "node03"]
    request_body = V0046OpenapiHostlistReqResp(hostlist="node[01-03]")

    response = util_api.util_v0046_post_hostnames(
        v0046_openapi_hostlist_req_resp=request_body
    )
...
def test_util_hostlist(util_api):
    from openapi_client.models.v0046_openapi_hostnames_req_resp import (
        V0046OpenapiHostnamesReqResp,
    )

    request_body = V0046OpenapiHostnamesReqResp(
        hostnames=["node01", "node02", "node03"]
    )
    response = util_api.util_v0046_post_hostlist(
        v0046_openapi_hostnames_req_resp=request_body
    )
...
def test_resv_crash(slurm, admin_level, cleanup_crash):
    ...
    users = ["root", "atf"]
    duration = V0046Uint32NoValStruct(number=1, set=True)
    start_time = V0046Uint64NoValStruct(
